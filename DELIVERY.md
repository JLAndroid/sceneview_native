...
**推荐通过网络 Maven 接入**，使用 JitPack 仓库 `https://jitpack.io`，声明 `com.github.JLAndroid.sceneview_native:sceneview-native:4.35.0`，详见 [README.md 的 JitPack 接入说明](README.md#通过-jitpack-maven-接入)。两个模块和第三方依赖会自动解析，无需手动复制 AAR。以下内容保留给需要离线分发或手动 AAR 接入的使用者；两种方式不要同时使用。
...
以下是 4.35.0 重新构建后的目标 AAR 文件名；尚未生成时不能把旧版 AAR 直接改名使用：

- `dist/aar/sceneview-native-4.35.0.aar`
- `dist/aar/sceneview-core-4.35.0.aar`
...
不要把整个 dist/aar 目录里的旧版 AAR 一起导入。当前版本文件名为 `4.35.0`。
...
若使用附带的 dist/repository，在 settings 中注册该仓库后声明 `implementation 'com.github.JLAndroid.sceneview_native:sceneview-native:4.35.0'` 即可自动带入 core 与第三方依赖，不再同时添加 app/libs 中的两个 AAR。
...
以下记录对应旧版 `4.35.0-native.3-kotlin1.9`，不是当前 `4.35.0` 的新构建验证。旧版实际发布的 Maven AAR 与直接 app/libs 的 AAR 均在 Kotlin 1.9.0 / AGP 8.2.2 下验证。详细结果见 VALIDATION.md 与 dist/BUILD-REPORT.json。尚未安装到设备，透明叠层、GPU 和生命周期运行行为仍需真机测试。原业务项目没有修改或构建。
