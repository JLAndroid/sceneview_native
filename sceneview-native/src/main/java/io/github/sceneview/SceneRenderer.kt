...
class SceneRenderer(
    ...
    fun renderFrame(
        ...
        if (swapChainRef.get() == null) return
        onBeforeRender()
        // A callback can detach the View or request destruction. Never retain the old surface.
    ...
    /** Temporarily detach a native View without destroying caller-owned mirror targets. */
    fun detach() {
        uiHelper.detach()
        swapChainRef.getAndSet(null)?.let {
