...
class ViewNode(
    ...
    // Detached constructor: Filament owns the GL context; no current UI-thread EGL context needed.
    private val surfaceTexture = createDetachedSurfaceTexture()
    ...
    class WindowManager(context: Context) {
        ...
        fun resume(ownerView: View) {
            ...
            if (ownerView.isAttachedToWindow) {
                ...
                ownerView.post {
                    ...
                    if (destroyed || !resumed) return@post
                    if (ownerView.isAttachedToWindow) {
                        clearPendingAttach()
                        tryAttachingView()
                    } else {
        ...
        private fun tryAttachingView() {
            if (destroyed || !resumed || layout.parent != null) return
            try {
