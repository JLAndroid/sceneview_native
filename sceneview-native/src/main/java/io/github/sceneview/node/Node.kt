...
import io.github.sceneview.gesture.OnContextClickListener
...
open class Node protected constructor(
    ...
    // Native frontend: independent subscriptions avoid replacing the single onFrame callback.
    private val frameListeners = mutableListOf<(Long) -> Unit>()
    private val ownedBindings = mutableListOf<AutoCloseable>()

    fun addFrameListener(listener: (Long) -> Unit): AutoCloseable {
        check(!isDestroyed)
        frameListeners += listener
        return AutoCloseable { frameListeners -= listener }
    }

    fun ownBinding(binding: AutoCloseable) {
        check(!isDestroyed)
    ...
    open fun onFrame(frameTimeNanos: Long) {
        ...
        childNodes.toList().forEach { if (!isDestroyed) it.onFrame(frameTimeNanos) }

        // User callback
        if (!isDestroyed) onFrame?.invoke(frameTimeNanos)
        frameListeners.toList().forEach { if (!isDestroyed) it(frameTimeNanos) }
    }
    ...
    open fun destroy() {
        if (isDestroyed) return
        isDestroyed = true
        ownedBindings.toList().forEach { runCatching { it.close() } }
        ownedBindings.clear()
