...
/**
 * Native state describing editing gestures. Construct once and read during onFrame.
 *
 * Registers its own listener and releases it with the node, or on explicit close().
 *
...
class NodeEditingFeedbackState(
...
) : NodeEditingListener, AutoCloseable {
    private var closed = false
    init {
        check(!node.isDestroyed)
        node.addEditingListener(this)
        node.ownBinding(this)
    }
    override fun close() {
        if (closed) return
        closed = true
        node.removeEditingListener(this)
    }

    /** The gestures currently active on [node] — twist and pinch can run simultaneously. */
    var activeKinds: Set<NodeEditingKind> = emptySet()
        private set
     ...
     * at ±90°.
     */
    var yawDegrees = quaternionYawDegrees(node.quaternion)
        private set

    /** Signed yaw accumulated since the current rotate gesture began, in degrees. */
    var rotationDeltaDegrees = 0f
        private set
    ...
    var scalePercent = percentOf(node.scale.x, scaleBaseline)
        private set
     ...
     */
    var scaleLimit: NodeScaleLimit? = null
        private set
    ...
    var moveWorldPosition: Position? = null
        private set
